...
- L'ancien dossier de compilation `dist` a été supprimé pour libérer C:, saturé pendant les vérifications. Il ne contenait aucun fichier suivi ; les sources, les images et les archives de provenance sont conservées. La compilation locale du code réussit dans `W:/Codex-Temp/lively-darwin-wave6-noncombat-20260831/dist`, sans copie des médias (`copyPublicDir: false`). Le build Vercel conserve sa copie normale des médias et toutes ses validations.
- Le premier candidat Linux a détecté que Vercel omettait la jonction Windows `public/backgrounds`. Les 589 fichiers (557 307 316 octets) sont désormais physiques dans le dépôt et ont été comparés octet par octet à leur source conservée sur E:. L'ancienne jonction est conservée sous `.tmp-backgrounds-junction-wave6-20260831`, hors publication. Le contrôle pré-envoi vérifie dorénavant tous les fichiers publics suivis par Git ; le test du prologue reste inchangé.
